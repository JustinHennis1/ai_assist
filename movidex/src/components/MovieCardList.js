...
function MovieCardList() {
  ...
  useEffect(() => {
    async function fetchTheaterMovies(page) {
      try {
        const response = await fetch(`http://localhost:5000/api/inTheaters?page=${page}`);
        const data = await response.json();
  ...
  useEffect(() => {
    async function fetchTopRatedMovies(page) {
      try {
        const response = await fetch(`http://localhost:5000/api/topRated?page=${page}`);
  ...
  useEffect(() => {
    async function fetchUpcomingMovies(page) {
      try {
        const response = await fetch(`http://localhost:5000/api/upcoming?page=${page}`);
        const data = await response.json();
